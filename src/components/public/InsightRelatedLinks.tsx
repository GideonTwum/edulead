...
interface InsightRelatedLinksProps {
  category?: string | null;
}

export function InsightRelatedLinks({ category }: InsightRelatedLinksProps) {
  ...
  if (normalized.includes("leadership") || normalized.includes("mentorship")) {
    links.push({
      ...
      label: "Explore youth leadership programmes",
    });
  }
  ...
  if (
    ...
    links.push({
      ...
      label: "Browse scholarships and youth opportunities",
    });
  }
  ...
  if (normalized.includes("policy") || normalized.includes("governance")) {
    links.push({
      href: ROUTES.events,
      label: "View youth policy dialogues and events",
    });
  }
